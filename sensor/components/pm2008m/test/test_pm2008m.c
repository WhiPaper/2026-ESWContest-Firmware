...
/* Include the implementation so its internal frame parser can be unit tested. */
#include "../pm2008m.c"

static void make_valid_frame(uint8_t frame[PM2008M_FRAME_SIZE])
{
    const uint8_t values[PM2008M_FRAME_SIZE - 1] = {
    ...
    frame[PM2008M_FRAME_SIZE - 1] = pm2008m_checksum(frame);
}
...
TEST_CASE("PM2008M rejects an invalid checksum", "[pm2008m]")
{
    ...
    pm2008m_data_t data;
    make_valid_frame(frame);
    frame[PM2008M_FRAME_SIZE - 1] ^= 0xff;
